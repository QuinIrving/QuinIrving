<div align="left">

```yaml
❯: whoami
┌──────────────────────────────────────────────────────────────────────────┐
  ID: QUIN IRVING                                                          │
  ROLE: SOFTWARE ENGINEER                                                  │
  TARGET: SYSTEMS | FULL_STACK | GAME ENGINE                               │
  STATUS: ONLINE                                                           │
└──────────────────────────────────────────────────────────────────────────┘
```

```yaml
❯: stack --list
- Systems Development & Performance
- Full-Stack (Front-End / Back-End)
- Data & Quant / SQL
```

```yaml
❯: cat dev_manifest.yml
ENGINEER_PROFILE:
  Focus: Low-latency systems, Memory optimization, Concurrency
  Philosophy: Write code that respects the hardware & performance.
  Capabilities:
    - Memory Management & RAII (C++)
    - High-Concurrency & Multithreading
    - Asynchronous Architecture (.NET/React)
    - Database Modeling & Query Optimization
  Active_Processes:
    - Optimizing engine sub-systems for maximum throughput
    - Researching zero-allocation patterns in C#
    - Designing scalable full-stack applications
```

```yaml
❯: system --stats
```
<table align="center">
  <tr>
    <th align="center">Languages</th>
    <th align="center">Frontend / Web</th>
    <th align="center">Databases</th>
  </tr>
  <tr>
    <td align="center">
      <a href="https://isocpp.org/"><img src="./assets/cpp.png" alt="C++" title="C++" height="42"></a>
      <a href="https://learn.microsoft.com/en-us/dotnet/csharp/"><img src="./assets/csharp.png" alt="C#" title="C#" height="42"></a>
      <a href="https://dev.java/"><img src="./assets/java.png" alt="Java" title="Java" height="42"></a>
      <a href="https://www.python.org/"><img src="./assets/python.png" alt="Python" title="Python" height="42"></a>
    </td>
    <td align="center">
      <a href="https://react.dev/"><img src="./assets/react.webp" alt="React" title="React" height="42"></a>
      <a href="https://www.typescriptlang.org/"><img src="./assets/typescript.png" alt="TypeScript" title="TypeScript" height="42"></a>
      <a href="https://www.w3.org/Style/CSS/Overview.en.html"><img src="./assets/css.png" alt="CSS3" title="CSS3" height="42"></a>
      <a href="https://html.spec.whatwg.org/"><img src="./assets/html.png" alt="HTML5" title="HTML5" height="42"></a>
    </td>
    <td align="center">
      <a href="https://www.mysql.com/"><img src="./assets/mysql.png" alt="MySQL" title="MySQL" height="42"></a>
      <a href="https://www.postgresql.org/"><img src="./assets/postgres.png" alt="PostgreSQL" title="PostgreSQL" height="42"></a>
    </td>
  </tr>
</table>



```yaml
❯: _
[ SESSION PERSISTED. NO MEMORY LEAKS DETECTED. ]
```
</div>
